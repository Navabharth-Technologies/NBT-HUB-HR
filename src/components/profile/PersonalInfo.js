...
export default function PersonalInfo({ onBack }) {
  ...
  const handleSave = async (shouldGoNext = false) => {
    ...
    const mandatoryFields = [
      ...
      { key: 'passbook_photo', label: 'Bank Passbook' }
    ];
    ...
    const separationVal = form.separation;
    if (activeSectionFields.includes('separation') && separationVal) {
      if (!/^\d{2}\/\d{2}\/\d{4}$/.test(separationVal) && !/^\d{2}-\d{2}-\d{4}$/.test(separationVal)) {
    ...
    try {
      ...
      // Format dates - date_of_birth and separation stay DD/MM/YYYY; other dates convert to YYYY-MM-DD
      const payload = { employee_id: uid, id: uid };
      activeSectionFields.forEach(k => {
        let val = form[k];
        if ((k === 'date_of_birth' || k === 'separation') && val && typeof val === 'string') {
        ...
        } else if (['doj', 'lwd', 'joining_date', 'date_of_joining'].includes(k) && val && typeof val === 'string') {
          if (val.includes('-')) {
            const parts = val.split('-');
            if (parts.length === 3 && parts[0].length === 2) {
              val = `${parts[2]}-${parts[1]}-${parts[0]}`;
                    ...
                    <div key={field.key} style={{ display: 'flex', flexDirection: 'column', gap: '8px', width: '100%', boxSizing: 'border-box' }}>
                      ...
                      {field.type === 'file' ? (
                        ...
                        <select
                          ...
                          style={{ width: '100%', padding: '16px 20px', borderRadius: '16px', fontWeight: form[field.key] ? '900' : '400', color: form[field.key] ? '#0B1E3F' : '#94a3b8', WebkitTextFillColor: form[field.key] ? '#0B1E3F' : '#94a3b8', border: isMobile ? '2px solid #cbd5e1' : '3px solid #cbd5e1', backgroundColor: isDisabled ? '#f1f5f9' : 'white', boxSizing: 'border-box', fontFamily: 'inherit', fontSize: '16px' }}
                        >
                          <option value="" disabled style={{ color: '#94a3b8', fontWeight: '400', WebkitTextFillColor: '#94a3b8' }}>
