...
export default function UpdatePasswordModal({ isOpen, onClose, userEmail }) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (response.ok) {
        // Immediately clear the stale token so no subsequent API calls
        // (from other components or background effects) can trigger a 401
        // 'globalLogout' modal during the 3-second success display window.
        localStorage.removeItem('token');
        localStorage.removeItem('navAuthUser');
        localStorage.removeItem('userRole');

        // Password was successfully changed — show success
        setSuccess(true);
        setError(null);
        setTimeout(() => {
          logout();
        ...
        // Server returned an error — show specific message
        const serverMsg = data.error || data.message || null;
        if (response.status >= 500) {
          setError(serverMsg || 'Database error. Please try again later.');
        } else if (response.status === 401 || response.status === 403) {
          setError(serverMsg || 'Incorrect current password. Please try again.');
        } else if (response.status === 404) {
          setError(serverMsg || 'User not found. Please contact support.');
        } else {
          setError(serverMsg || 'Failed to update password. Please try again.');
        }
      }
    } catch (err) {
