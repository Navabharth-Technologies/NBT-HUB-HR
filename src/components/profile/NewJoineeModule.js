...
export default function NewJoineeModule() {
  ...
  const fetchJoinees = async () => {
    ...
    try {
      ...
      setJoinees([...(Array.isArray(joineeData) ? joineeData : []), ...normalizedInterns]);
    } catch (err) {
  ...
  const handleEdit = (joinee) => {
    ...
    const dbId = joinee.id || joinee._id || joinee.id_intern;
    setEditingId(dbId);
    ...
    const existingManager = leads.find(l => {
      ...
      const dbId = String(joinee.Reporting_manager_id || joinee.reporting_manager_id || '');
      ...
      return (leadId !== '' && leadId === dbId) || (leadName !== '' && leadName === dbName);
    });
    ...
    setFormData({
      ...
      employee_id: joinee.employee_id || joinee.intern_id || '',
      reporting_manager: existingManager ? (existingManager.employee_id || existingManager.name) : (joinee.Reporting_manager_id || joinee.reporting_manager_id || joinee.Reporting_manager || joinee.reporting_manager || ''),
      phone_number: joinee.phone_number || joinee.phone || '',
      duration_months: joinee.duration_months || '',
  ...
  const handleAddJoinee = async (e) => {
    ...
    const payload = isIntern ? {
      ...
      stipend: 0,
      duration_months: formData.duration_months || 6,
      status: 'Active',
    ...
    try {
      ...
      if (response.ok) {
        ...
        setEditingId(null);
        setFormData({ name: '', email_id: '', role: '', joining_date: '', employee_id: '', reporting_manager: '', phone_number: '', duration_months: '', is_intern: false });
        fetchJoinees();
      } else {
            ...
            <div style={{ position: 'relative' }}>
              ...
              {showAddDropdown && (
                <div style={{
                  ...
                  <button
                    onClick={() => { setShowAddModal(true); setEditingId(null); setShowAddDropdown(false); setFormData({ name: '', email_id: '', role: '', joining_date: '', employee_id: '', reporting_manager: '', phone_number: '', duration_months: '', is_intern: false }); }}
                    style={{
                  ...
                  <button
                    onClick={() => { setShowAddModal(true); setEditingId(null); setShowAddDropdown(false); setFormData({ name: '', email_id: '', role: 'Intern', joining_date: '', employee_id: '', reporting_manager: '', phone_number: '', duration_months: '', is_intern: true }); }}
                    style={{
                      width: '100%', padding: '12px 16px', textAlign: 'left', border: 'none',
                      background: 'transparent', cursor: 'pointer', borderRadius: '10px',
                      fontWeight: '700', color: '#1e293b', fontSize: '14px', transition: '0.2s'
            ...
            filteredJoinees.map((joinee, i) => (
              <div key={i} className="team-card" style={{ background: 'white', padding: '24px', borderRadius: '24px', border: '1px solid #eef2f6', cursor: 'pointer' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '20px' }}>
                  ...
                  {joinee.duration_months && (
                    <div style={{ display: 'flex', gap: '10px', fontSize: '13px', alignItems: 'center' }}>
                      <span style={{ color: '#94a3b8', minWidth: '110px' }}>Duration</span>
                      <span style={{ fontWeight: '700', color: '#f59e0b' }}>{joinee.duration_months} Months</span>
                    </div>
                  )}
                </div>
            ...
            >
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '16px' }}>
                ...
                <h2 style={{ fontSize: '24px', fontWeight: '900', color: '#0B1E3F', margin: '0 0 4px 0', textAlign: 'center', letterSpacing: '-0.5px' }}>
                  {editingId ? 'Edit information' : (formData.role === 'Intern' ? 'Enroll new Intern' : 'Enroll new employee')}
                </h2>
              ...
              <form onSubmit={handleAddJoinee} style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '14px 20px', width: '100%', boxSizing: 'border-box' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  <label style={{ fontSize: '12px', fontWeight: '900', color: '#1e293b', letterSpacing: '0.3px' }}>
                    {formData.role === 'Intern' ? 'Intern ID' : 'Employee ID'}
                  </label>
                  <input
                    type="text" name="employee_id" value={formData.employee_id} onChange={handleInputChange}
                    placeholder={formData.role === 'Intern' ? 'e.g. INT-001' : 'e.g. NBT-001'} required
                    style={{ width: '100%', boxSizing: 'border-box', padding: '12px 18px', borderRadius: '14px', border: '2px solid #e2e8f0', background: '#f8fafc', fontWeight: '800', fontSize: '13px', outline: 'none', color: '#0B1E3F' }}
                  />
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  <label style={{ fontSize: '12px', fontWeight: '900', color: '#1e293b', letterSpacing: '0.3px' }}>Duration (Months)</label>
                  <input
                    type="number" name="duration_months" value={formData.duration_months} onChange={handleInputChange}
