...
const calcPanelPosition = (iconRect, winWidth, winHeight, isMobile) => {
  ...
  // Space above and below the icon (within safe area)
  const spaceAbove = iconRect.top - headerBottom - GAP;
  const spaceBelow = footerTop - iconRect.bottom - GAP;

  // Prefer opening above if there's more room, else below
  ...
  // Horizontal: align right edge with icon right edge; clamp so panel doesn't go off-screen
...
const TaskNotification = ({ onOpenTask }) => {
  ...
  // Compute drag constraints (icon must stay between header and footer)
  ...
  // Compute panel position based on current icon location
  ...
  // Also update panel position when panel opens
  useEffect(() => {
  ...
  return (
    <>
      {/* ── Notification Panel (fixed, always within viewport) ── */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
            key="notif-panel"
                ...
                <div key={notif.id} style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                  ...
                  <div
                    ...
                    style={{
                      ...
                      padding: '14px',
                      borderRadius: '18px',
                      border: notif.isNew ? '1.5px solid #3B599825' : '1.5px solid #f1f5f9',
                      boxShadow: notif.isNew ? '0 6px 16px rgba(59,89,152,0.07)' : 'none',
                      cursor: 'pointer',
