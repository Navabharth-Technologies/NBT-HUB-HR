...
export default function AwardsScreen() {
    ...
    const fetchInitialData = async () => {
        ...
        try {
                ...
                if (allRes.ok && rewHistoryRes.ok) {
                    ...
                    const cumulativeQuizPoints = employeesList.map(emp => {
                        ...
                        return {
                            employee_id: empId,
                            points: quizPoints,
                            created_at: emp.completion_date || emp.quiz_completion_date || emp.created_at || emp.date || emp.quiz_date || emp.timestamp || null
                        };
                        ...
                        try {
                            ...
                            datedQuizPoints = rawList.map(item => {
                                return {
                                    ...
                                    created_at: item.created_at || item.completion_date || item.date || item.quiz_date || item.timestamp || null,
                                    quiz_title: item.quiz_title || item.quiz_name || 'Quiz Excellence'
                                };
                            }).filter(item => item.employee_id && item.points > 0);
                        } catch (e) {
            ...
            // Key: empId-YYYY-MM-DD per dated row. No undated 'cumulative' override that destroys history.
            ...
            if (fetchSuccess) {
                try {
                    // Load existing localStorage cache
                    ...
                    // Build merged map from cache first (seed with all history)
                    ...
                    // Overlay fresh API records — add new ones, never remove existing
                    qList.forEach(item => {
                        if (!item) return;
                        const empId = String(item.employee_id || item.user_id || item.userId || item.id || '');
                        if (!empId) return;
                ...
                // Network failed — fall back to full localStorage cache
                try {
                    const localData = localStorage.getItem('nbt_historical_quiz_scores');
                    if (localData) {
                        const parsed = JSON.parse(localData);
